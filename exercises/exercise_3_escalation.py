    ...
    with console.status("[dim]LLM reviewing the diff...[/dim]"):
        analysis = llm.invoke([
            {"role": "system", "content": (
                ...
                "If confidence < 0.60, populate escalation_questions "
...
def node_synthesize(state: ReviewState) -> dict:
    ...
    prompt = f"""Original Title: {state['pr_title']}
Original Diff: {state['pr_diff']}
Initial Analysis: {state['analysis'].model_dump_json()}
Reviewer Answers:
{answers_str}
Synthesize a refined PRAnalysis using the reviewer answers."""
