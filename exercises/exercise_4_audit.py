...
async def audit(state, entry: AuditEntry) -> None:
    ...
    the entry itself.
    """
    await write_audit_event(thread_id=state["thread_id"],
...
async def node_human_approval(state):
    ...
    await audit(state, AuditEntry(
        ...
        confidence=a.confidence,
        risk_level=risk_level_for(a.confidence),
        reviewer_id=os.environ.get("GITHUB_USER", "unknown"),
        decision=resp.get("choice", "pending"),
        reason=resp.get("feedback"),
        execution_time_ms=int((time.monotonic() - t0_resume) * 1000),
    ))
...
async def node_commit(state):
    ...
    await audit(state, AuditEntry(
        ...
        decision=state.get("human_choice", "auto"),
        reason=action,
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
...
async def node_escalate(state):
    ...
    await audit(state, AuditEntry(
        ...
        risk_level=risk_level_for(a.confidence),
        decision="pending",
        reason="Waiting for escalation answers",
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
