...
with st.sidebar:
    ...
    try:
        ...
        df = pd.read_sql("""
            SELECT thread_id, pr_url, worst_risk, last_event
            FROM (
        ...
        for _, row in df.iterrows():
            if st.button(f"Thread {row['thread_id'][:6]}", key=f"btn_{row['thread_id']}"):
...
def render_escalation_card(payload: dict) -> dict | None:
    ...
    with st.form("escalation"):
        ...
        submitted_escalation = st.form_submit_button("Submit answers")
